...
# create a table for each cluster that has the total counts for each unique value
cluster_1_table <- apply(cluster_1_rows, 2, table)
cluster_2_table <- apply(cluster_2_rows, 2, table)
cluster_3_table <- apply(cluster_3_rows, 2, table)

# adding values that are missin but are in other clusters to the tables with a count of 0
# get the unique values for each variable
unique_vals <- lapply(linear_dat, unique)

# get the unique values for each variable in each cluster
unique_vals_1 <- lapply(cluster_1_rows, unique)
unique_vals_2 <- lapply(cluster_2_rows, unique)
unique_vals_3 <- lapply(cluster_3_rows, unique)

# get the values that are in the unique values for each variable but not in the unique values for each cluster
# and add them to the tables with a count of 0
for (i in 1:length(unique_vals)) {
  # get the variable name
  var_name <- names(unique_vals)[i]
  # get the unique values for the variable
  var_vals <- unique_vals[[i]]
  # get the unique values for the variable in each cluster
  var_vals_1 <- unique_vals_1[[i]]
  var_vals_2 <- unique_vals_2[[i]]
  var_vals_3 <- unique_vals_3[[i]]
  # get the values that are in the unique values for the variable but not in the unique values for the cluster
  # and add them to the tables with a count of 0
  for (j in 1:length(var_vals)) {
    # get the value
    val <- var_vals[j]
    # check if the value is in the unique values for the variable in each cluster
    if (!(val %in% var_vals_1)) {
      # add the value to the table with a count of 0
      cluster_1_table[[var_name]][[as.character(val)]] <- 0
    }
    if (!(val %in% var_vals_2)) {
      # add the value to the table with a count of 0
      cluster_2_table[[var_name]][[as.character(val)]] <- 0
    }
    if (!(val %in% var_vals_3)) {
      # add the value to the table with a count of 0
      cluster_3_table[[var_name]][[as.character(val)]] <- 0
    }
  }
}

# merge the tables into one dataframe
cluster_tables <- cbind(cluster_1_table, cluster_2_table, cluster_3_table)
...
# rename the columns
xyz_1 <- rename(xyz_1, lepain = Var1, Freq = Freq)
xyz_2 <- rename(xyz_2, lepain = Var1, Freq = Freq)
xyz_3 <- rename(xyz_3, lepain = Var1, Freq = Freq)
...
# plot adjacent bars for each value for each cluster
ggplot(xyz_dat, aes(x = cluster, y = Freq, fill = xyz)) +
  geom_bar(stat = "identity", position = "dodge") +
...
logreg_dat <- temp[temp$employment_level != "(-1) Blank", ]
logreg_dat$employment_level <- drop_na(logreg_dat$employment_level)
...
### GLM Model
```{r logreg model}
logreg <- glm(employment_level ~ ., data = logreg_dat)
```
```{r}
